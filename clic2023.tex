...
\newcite{caselli-etal-2021-hatebert} retrain BERT$_{base}$ on the MLM task using an unlabelled corpus built from hateful and offensive Reddit messages, obtaining a model called HateBERT, capable of outperforming BERT$_{base}$ on hate speech identification on various benchmark datasets.
...
\begin{table*}[t]
  \centering
  % \footnotesize
  \caption{Performance when fine-tuning the monolingual models on Italian-only corpora. Epochs (e) selected based on val. F$_1$. Best scores in bold, second-best underlined; \bs\, = corpus used for training.}
...
\begin{table*}[t]
  \centering
  % \footnotesize
  \caption{Performance when fine-tuning the multilingual models on mono- and bilingual corpora combinations. Epochs (e) selected based on val. F$_1$. Best scores in bold; \bs\, = corpus used for training.}
