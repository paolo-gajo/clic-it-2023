...
\begin{table}
    \centering
    \caption{Performance of an \imbert\, model when fine-tuned on \dsENclassification\, and \ami, on different subsets of the \dsITclassification\, test corpus. M~$=$~Misogynous, R~$=$~Racist.}
    \label{tab:misogyny-racism-subsets-statistics}
    \begin{tabular}{ccccc}
      \hline
      \bf Prec & \bf Rec & \bf F$_1$ & \bf Instances & \bf Source\\
      \hline
      0.784 & 0.670 & 0.722 & 500 & All        \\
      1.000 & 0.800 & 0.889 & 5 & M $\cup$ R \\
      1.000 & 0.875 & 0.933 & 8 & R          \\
      1.000 & 0.658 & 0.794 & 188 & M          \\
      \hline
  \end{tabular}      
\end{table}
\todoP{I think it looks a bit weird to have this paragraph without a paragraph\{\} at the beginning}
\paolo{
Table~\ref{tab:misogyny-racism-subsets-statistics} reports the statistics for the performance on the individual ``misogynous'' and ``racist'' labels of the \dsITclassification\, test set, for an \imbert\, model newly fine-tuned on both the \dsENclassification\, and \ami\, corpora.
Expectedly, since we are calculating the F$_1$-measure on the positive class, the peformance on the positive label subsets is much higher than on the whole dataset.
...
The explanation for this is possibly two-fold: 1) the number of instances which are only racist is much smaller (8 vs. 188) and 2) compared to the misogyny expressed by \itforum\, users, the racism is much more explicit and easier to detect.
...
In such cases, although users might be even auto-censoring themselves by substituting letters with numbers, the model can still detect the hateful language.
