...
\newcite{mathew2021hatexplain} build a corpus called HateXplain from Twitter and Gab posts, annotated with a multi-class label based on whether the post is ``offensive'', expresses ``hate'', or is ``normal'', which they use to fine-tune a BERT hate speech classifier.
...
In multilingual settings, \newcite{peliconInvestigatingCrosslingualTraining2021} use a multilingual combination of corpora annotated for hate speech to improve the performance of classifiers in zero-shot, few-shot and well-resourced settings. \newcite{gokhaleSpreadLoveNot2022} use MLM training to improve the hate speech detection performance of BERT in Hindi and Marathi, separately. We follow such approaches in attempting to improve the performance of our models\paolo{, with a specific focus on monolingual vs. bilingual pre-training, compared to \newcite{gajo2023identification}.}
...
The intersection between \dsITclassification\, and the Italian data used for MLM training is void; the same is true for \dsENclassification\, and the English MLM training data.
\medskip
...
\begin{table*}[t]
  \centering
  % \footnotesize
  \caption{Performance when fine-tuning the multilingual models on monolingual and bilingual combinations of English and Italian supervised corpora. Epochs (e) selected based on validation performance. Highest scores in bold.}
